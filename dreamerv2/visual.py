...
for angle in tqdm(range(0, 360, 5)):
  ...
  infer_false_1 = agnt.wm.partial_imagination(data, infer_start=False)
  infer_false_2 = agnt.wm.partial_imagination(data, infer_start=False)
  infer_false_3 = agnt.wm.partial_imagination(data, infer_start=False)
  infer_true = agnt.wm.partial_imagination(data, infer_start=True)
  T, H, BW, C = infer_false_1.shape
  gt = infer_false_1[:, :H // 3]
  recon_delta_1 = infer_false_1[:, (H // 3):]
  recon_delta_2 = infer_false_2[:, (H // 3):]
  recon_delta_3 = infer_false_3[:, (H // 3):]
  recon_delta_4 = infer_true[:, (H // 3):]
  video = tf.concat([gt, recon_delta_1, recon_delta_2, recon_delta_3, recon_delta_4], 1)
  video = tf.concat(tf.split(video, C // 3, 3), 1)

  #infer_false = (np.transpose(infer_false, (0, 3, 1, 2)) * 255).astype(np.uint8)
  #infer_true = (np.transpose(infer_true, (0, 3, 1, 2)) * 255).astype(np.uint8)
  video = (np.transpose(video, (0, 3, 1, 2)) * 255).astype(np.uint8)
  #wandb.log({f"obj_blind_from_start": wandb.Video(infer_false, fps=30, format="gif")})
  #wandb.log({f"obj_blind_after_start": wandb.Video(infer_true, fps=30, format="gif")})
  wandb.log({f"obj_blind": wandb.Video(video, fps=30, format="gif")})
