import atexit
import os
import functools
import sys
sys.path.insert(0, '..')
import threading
import traceback

import gym
import numpy as np
from PIL import Image

from .wrappers import FrameSkip, EpisodeLengthWrapper, ThreeJointsOneHot, ThreeJointsManyHot, ThreeJointsCont

def build_env(env_config=None):
  from .environment.kitchen_env import KitchenEnv
  from omni.isaac.synthetic_utils import OmniKitHelper
  os.environ['CUDA_VISIBLE_DEVICES'] = '0'
  if env_config is None:
    env_config = {}
  w, h = env_config.get('width', 96), env_config.get('height', 64)
  CUSTOM_CONFIG = {
    "width": 320,
    "height": 240,
    "renderer": "RayTracedLighting",
    "headless": True,
    "experience": f'{os.environ["EXP_PATH"]}/isaac-sim-python.json',
  }
  omniverse_kit = OmniKitHelper(CUSTOM_CONFIG)
  omniverse_kit.set_setting("/rtx/post/aa/op", 0)

  env = KitchenEnv(
    omni_kit=omniverse_kit,
    robot_path="data/urdf/robots/husky_ur5/husky_ur5_1.urdf",
    sparse_reward=False,
    width=w, height=h,
    env_config_file=env_config.get('env_config_file', "./environment/env_config.yml")
  )
  env.start()
  env = FrameSkip(env, skip=2)
  ml = env_config.get('episode_steps', 1000)
  env = EpisodeLengthWrapper(env, max_length=ml)
  dist = env_config.get('action_dist', 'onehot')
  if dist == 'onehot':
    env = ThreeJointsOneHot(env)
  elif dist == 'manyhot':
    env = ThreeJointsManyHot(env)
  elif dist == 'cont':
    env = ThreeJointsCont(env)
  return env

class Isaac:

  #LOCK = threading.Lock()

  def __init__(
      self, name, action_repeat=4, size=(64, 96), grayscale=False, noops=30,
      life_done=False, onehot=True, episode_steps=1000, sticky_actions=True,
      env_config_file="./environment/env_config.yml"):
    version = 0 if sticky_actions else 4
    name = ''.join(word.title() for word in name.split('_'))
    #with self.LOCK:
    self._env = build_env({
        'action_dist': 'onehot' if onehot else 'cont',
        'episode_steps': episode_steps,
        'width': size[1], 'height': size[0],
        'env_config_file': env_config_file,
    })
    self._action_repeat = action_repeat
    self._size = size
    self._grayscale = grayscale
    self._noops = noops
    self._life_done = life_done
    self._lives = None
    shape = self._env.observation_space.shape[:2] + (() if grayscale else (3,))
    self._buffers = [np.empty(shape, dtype=np.uint8) for _ in range(2)]
    self._random = np.random.RandomState(seed=None)

  @property
  def observation_space(self):
    shape = self._size + (1 if self._grayscale else 3,)
    space = gym.spaces.Box(low=0, high=255, shape=shape, dtype=np.uint8)
    return gym.spaces.Dict({'image': space})

  @property
  def action_space(self):
    return self._env.action_space

  def close(self):
    return self._env.close()

  def reset(self):
    #with self.LOCK:
    self._env.reset()
    #noops = self._random.randint(1, self._noops + 1)
    #for _ in range(noops):
    #  done = self._env.step(0)[2]
    #  if done:
    #    with self.LOCK:
    #      self._env.reset()
    #self._lives = self._env.ale.lives()
    #if self._grayscale:
    #  self._env.ale.getScreenGrayscale(self._buffers[0])
    #else:
    #  self._env.ale.getScreenRGB2(self._buffers[0])
    #self._buffers[1].fill(0)
    return self._get_obs()

  def step(self, action):
    obs, reward, done, info = self._env.step(action)
    obs = np.transpose(obs, (1, 2, 0))
    info['original'] = np.transpose(info['original'], (1, 2, 0))
    obs = {'image': obs, 'original': info['original']}
    return obs, reward, done, info
    #total_reward = 0.0
    #for step in range(self._action_repeat):
    #  _, reward, done, info = self._env.step(action)
    #  total_reward += reward
    #  if self._life_done:
    #    lives = self._env.ale.lives()
    #    done = done or lives < self._lives
    #    self._lives = lives
    #  if done:
    #    break
    #  elif step >= self._action_repeat - 2:
    #    index = step - (self._action_repeat - 2)
    #    if self._grayscale:
    #      self._env.ale.getScreenGrayscale(self._buffers[index])
    #    else:
    #      self._env.ale.getScreenRGB2(self._buffers[index])
    #obs = self._get_obs()
    #return obs, total_reward, done, info

  def render(self, mode):
    return self._env.render(mode)

  def _get_obs(self):
    obs = self._env.get_obs()[0]
    obs = np.transpose(obs, (1, 2, 0))
    return {'image': obs}
    #if self._action_repeat > 1:
    #  np.maximum(self._buffers[0], self._buffers[1], out=self._buffers[0])
    #image = np.array(Image.fromarray(self._buffers[0]).resize(
    #    self._size, Image.BILINEAR))
    #image = np.clip(image, 0, 255).astype(np.uint8)
    #image = image[:, :, None] if self._grayscale else image
    #return {'image': image}