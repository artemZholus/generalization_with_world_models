...
class DualReasoner(RSSM):
  ...
  def top_down_step(self, state, obj_emb=None, subj_emb=None, action=None, current_step=None, task_vec=None, sample=True):
    ...
    if current_step is not None:
      subj_curr_state = current_step['subj']
      obj_curr_state = current_step['obj']
    else:
    ...
    obj_post = self.obj_reasoner.obs_step(prev_state=obj_state,
                                          ...
                                          post_update=obj_emb,
                                          sample=sample)
    util_cond = {k: tf.concat([self._cast(obj_post[k]), self._cast(obj_state[k])], 1) for k in obj_post.keys()}
    utility = self.condition_model.observe(self.obj_reasoner.get_feat(util_cond), sample=sample)
